...
export function EnvelopeGate() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-pink-200 via-rose-100 to-pink-300 overflow-hidden">
      <AnimatePresence>
        {!showNext && (
          <motion.div
            animate={
              unlocked ? { y: -500, opacity: 0, scale: 0.8 } : { y: 0, opacity: 1 }
            }
                ...
                >
                  ...
                  <div className="relative w-full">
                    <motion.input
                      ...
                      className={`w-full px-3 py-2 pr-10 rounded-md text-sm bg-white text-pink-700 border outline-none
                        ${error ? "border-red-400" : "border-pink-300"}`}
                    />

                    <div
                      onClick={() => setShowPassword((prev) => !prev)}
                      className="absolute inset-y-0 right-2 flex items-center text-pink-500 text-sm select-none"
                    >
                      {showPassword ? "🙈" : "👁️"}
                    </div>
                  </div>
      ...
      <AnimatePresence>
        {showNext && (
          <div className="flex flex-col items-center gap-4">
            <p className="text-pink-700 font-semibold">Unlocked</p>
            <button
