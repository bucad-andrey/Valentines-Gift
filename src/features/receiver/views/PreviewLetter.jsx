...
export function PreviewLetter({ userId }) {
  ...
  useEffect(() => {
    ...
    const docRef = doc(db, "Senders", userId, "message1", "message");
  ...
  useEffect(() => {
    ...
    typingIntervalRef.current = setInterval(() => {
      ...
      if (index >= currentText.length) clearInterval(typingIntervalRef.current);
    }, 60);
  ...
  return (
    <section className="bg-linear-to-br from-primary-soft to-secondary-soft w-full min-h-screen md:flex flex-col items-center">
      <div className="block lg:flex justify-center items-center relative">
        ...
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
              h-[500px] w-screen md:w-[400px] p-16 mg:p-8 pt-16 md:pt-3
              font-[Patrick_Hand] text-lg leading-6 mg:leading-8 tracking-wide text-black whitespace-pre-wrap"
        >
          {animatedText}
        </div>
      </div>

      <div className="flex gap-4 mt-10 overflow-scroll relative md:absolute md:bottom-1 md:overflow-hidden">
        <button
          onClick={handleSkip}
