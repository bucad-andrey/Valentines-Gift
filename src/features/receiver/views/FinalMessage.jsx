...
import {
  ...
  saveFinalResponse,
} from "../../../shared/lib/firestore/firestoreHelpers";
...
export function FinalMessage({ userId }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!latest) setError("No invitation found yet. Create one first.");
        else setData(latest);
      } catch (err) {
  ...
  const youtubeId = useMemo(
    () => extractYouTubeId(data?.soundtrackUrl || ""),
    [data?.soundtrackUrl]
  );
  ...
  void auth; // kept for parity; safe to remove later
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2">
            <div className="relative">
              ...
              <div className="absolute bottom-5 left-5 right-5">
                ...
                {whenPretty && (
                  <p className="mt-2 text-white/90 text-sm sm:text-base">{whenPretty}</p>
                )}
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8 space-y-6">
              ...
              {whereImages?.filter(Boolean)?.length > 1 && (
                <div className="space-y-2">
                  <p className="text-sm font-semibold text-primary-text">Location options</p>
              ...
              {(youtubeId || spotifyEmbed || data?.soundtrackUrl) && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    <p className="text-sm font-semibold text-primary-text">Soundtrack</p>
                    {data?.soundtrackUrl && (
                      <a
              ...
              <div className="space-y-3 pt-2">
                <p className="text-sm font-semibold text-primary-text">Will you come?</p>
